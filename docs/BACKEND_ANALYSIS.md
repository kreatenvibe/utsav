...
- **Ways to get a `users` row**: `POST /colonies/:id/members` and `POST /colonies/:id/members/bulk` (create-or-link — creates a new row when the given phone doesn't already resolve to one, see §4/§6), plus `POST /auth/bootstrap` for exactly one row — the very first, on a deployment with no `users` at all (§4/§5). No other creation path exists. **`POST /auth/register` no longer exists** (migration 016) — self-registration has been removed entirely; every account other than the one-time bootstrap account is created by a colony admin. (Migration 014's one-time backfill of placeholder accounts for pre-`members`-retirement rows is historical and no longer a live creation path.)
...
- **Bootstrapping the very first account**: `POST /auth/bootstrap` (§4/§5) creates it — public, no auth, works exactly once per deployment (403 once any `users` row exists). Every account after that is still admin-created via `POST /colonies/:id/members` or its bulk variant.
...
|---|---|---|---|
| POST | `/auth/bootstrap` | One-time: create the very first user account in a fresh deployment | — |
| POST | `/auth/login` | Log in, get a JWT | — |
| PATCH | `/auth/change-password` | Self-service password change | 🔒 |

- **`POST /auth/register` no longer exists** (migration 016) — a `POST` to it now falls through to the app-wide write gate (401 unauthenticated, 404 if authenticated — no route matches). There is no self-registration path of any kind; every `users` row (other than the one-time bootstrap account below) is created by a colony admin via `POST /colonies/:id/members` or its bulk variant (see Colonies below).
- **Bootstrap** (new — closes the gap flagged throughout this doc since migration 016 removed self-registration): body `{ name, phone, password }`, same three fields `POST /colonies/:id/members` requires to create a brand-new account. 400 `"name, phone, and password are required"` if any is missing. **Only succeeds while the `users` table is completely empty** — it locks the table for the duration of one transaction (`LOCK TABLE users IN EXCLUSIVE MODE`) so two near-simultaneous calls can't both pass the empty-check and both insert; the loser sees a non-empty table and gets 403 `"setup already completed"`. Response `201`: `{ token }` — same shape and JWT payload as `POST /auth/login`, so a mobile "first-run setup" screen can go straight from this call into the authenticated app with no separate login step. **Not a platform role**: the resulting account is a plain `users` row, identical to any other — no special flag is set, and this app has no platform-admin concept. It's simply the account that's positioned to call `POST /colonies` and become admin of whatever colony it creates; every account after that is onboarded the normal way (a colony admin calling `POST /colonies/:id/members`). Bootstrap never succeeds again once any account exists, including the one it just created.
...
- **Registration: removed entirely (migration 016).** `POST /auth/register` no longer exists — there is no self-registration path of any kind. Every `users` row is created by a colony admin via `POST /colonies/:id/members` or its bulk variant (§4), except for the one-time bootstrap account below. `services/authService.js`'s `registerUser` function was deleted along with the route (no remaining caller).
- **Bootstrap** (`POST /auth/bootstrap`, new — resolves the bootstrapping gap this section used to flag): the one exception to "every account is admin-created." Public, no auth. `{ name, phone, password }` in, `{ token }` out (same shape as login — it logs the new account straight in). Works exactly once per deployment: it only succeeds while `users` is completely empty, enforced by locking the table for one transaction (`LOCK TABLE users IN EXCLUSIVE MODE` around a count-check-then-insert) so two racing calls can't both slip through the empty check — the loser gets 403 `"setup already completed"`. The account it creates has no special role or flag; it's a plain `users` row that's simply first, so it's the one that calls `POST /colonies` and becomes that colony's admin. There is no separate "is setup needed?" status endpoint — a mobile first-run screen has to infer it from `POST /auth/bootstrap`'s own response (201 vs. 403 `"setup already completed"`), since that's the only server-side signal this backend exposes.
...
```
Fresh deployment only: POST /auth/bootstrap creates the first account and
logs it in directly (201 + token) — 403 "setup already completed" on any
deployment that already has a user, which is how a first-run screen tells
"needs setup" apart from "show Login instead"
  ↓ (every subsequent account, including on an already-bootstrapped deployment)
...
1. A user account exists and logs in — for the very first account in a fresh deployment, `POST /auth/bootstrap` creates and logs it in in one call (§3/§4/§5); every account after that is created by a colony admin instead.
...
| Bootstrap missing `name`/`phone`/`password` | 400 | `"name, phone, and password are required"` | Manual check in `authService.bootstrapFirstUser` |
| Bootstrap attempted on a deployment that already has a user | 403 | `"setup already completed"` | `authService.bootstrapFirstUser` — checked inside a `LOCK TABLE users`-guarded transaction so a race between two near-simultaneous calls can't let both through |
...
- **The very-first-account gap is resolved: `POST /auth/bootstrap`.** Public, no auth, `{ name, phone, password }` → `{ token }`, works exactly once per deployment (403 `"setup already completed"` once any `users` row exists — see §3/§4/§5). **Mobile consequence**: this is the one case where the mobile app needs a screen that isn't "Login" or an admin-managed member add — a first-run "Setup" screen shown only when the deployment is fresh, using bootstrap's own 201-vs-403 response as the signal for whether setup is still needed (there's no separate status endpoint).
...
- ~~How is the very first account in a fresh deployment actually meant to be provisioned?~~ **Resolved**: `POST /auth/bootstrap` (§3/§4/§5) — an API answer now exists, no manual `INSERT` needed.
