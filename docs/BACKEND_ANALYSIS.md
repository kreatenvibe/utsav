...
**users** (`migrations/008`, extended `migrations/012`)
- `user_id` (PK, serial), `email` (**nullable** as of migration 012, unique when present), `phone` (**nullable**, added migration 012, unique when present via a partial index), `password_hash` (required, bcrypt), `created_at` (auto).
...
- **Two ways to get a `users` row**: `POST /auth/register` (email+password only — self-registration is deliberately never phone-based) and `POST /members/bulk` (email- or phone-keyed, see §4/§6 — the only path that creates a phone-only login).
- No update/delete endpoints exist for `users` themselves. `PATCH /auth/change-password` (migration-011 feature) is the only self-service mutation. `GET /users?search=` (see §4) is the only read endpoint; there is still no create/update/delete path for a `users` row other than `/auth/register` and the bulk-import phone-login path.
- **No `name` column** — this is the complete schema: `user_id`, `email`, `phone`, `password_hash`, `created_at`. Anything that looks like it needs a display name for a user (e.g. a search-result list) currently has only `email`/`phone` to show.
...
- **Register** body: `{ email, password }`. Response `201`: `{ user_id, email }` (no password hash, no token). 400 if either field missing. 409 if email already registered. **Unchanged by migration 012** — self-registration is deliberately email+password only; there is no `POST /auth/register` path to a phone-only account. Phone-based logins are created exclusively via `POST /members/bulk` (see Members below).
...
- Added specifically so a colony admin can find a registered user's exact `email` before calling `POST /colonies/:id/members`, which previously required knowing that email exactly (404 if it didn't match anything).
- `search` is optional. Omit it and the endpoint returns every registered user (same always-a-full-list convention as every other GET in this API — still no pagination). Given, it matches **partial, case-insensitive against `email` OR `phone`** (migration 012 — was email-only before `users.phone` existed).
- Response is `[{ user_id, email, phone }]` (migration 012 added `phone`) — never `password_hash`, never any other column. Either `email` or `phone` may be `null` on a given row.
- Requires auth on the GET, same reasoning as `/colonies/mine` and `/colonies/:id/members` below: an unauthenticated user-search-by-email/phone endpoint would let anyone enumerate registered accounts.
...
**Bulk import (migration 011, extended migration 012)** — `multipart/form-data`: a `file` field (`.csv` or `.xlsx`/`.xls`, 5MB cap), plus form fields `colony_id` (required) and `initial_password` (required). Unlike single-create, `colony_id` here is **always required and always colony-admin-gated** — a bulk roster upload is inherently "the organizer's own colony," so there's no unscoped mode. File columns: `name` (required per row — stricter than single-create, since the dedup check needs a phone), `phone` (required per row), `email` (optional — a non-empty value signals "grant this person a login via email," using `initial_password`), `password` (optional, overrides `initial_password` for that one row only), **`grant_login`** (new, migration 012 — optional, truthy values `yes`/`true`/`1` case-insensitive; only consulted when `email` is empty). Row logic: `email` non-empty → grants a login keyed on `email` (unchanged from migration 011). `email` empty and `grant_login` truthy → grants a login keyed on the row's own `phone` instead, same `password`/`initial_password` fallback. `email` empty and `grant_login` falsy → plain roster row, no login (the common "volunteer" case, unchanged). Rows are processed independently — one bad row doesn't fail the batch — but a row's member-creation, (if requested) login-grant, and (migration 012, new) colony-membership auto-link all happen together, atomically.

**Colony auto-link (migration 012, the bug this fixed)**: whichever path grants a login (email or phone) also inserts a `colony_memberships` row for that user scoped to the batch's `colony_id`, role `'member'` (`ON CONFLICT (colony_id, user_id) DO NOTHING`, defensive). Before migration 012, an email-granted bulk login never got colony access — the person wouldn't appear under `GET /colonies/mine` for their own colony until an admin separately ran `PATCH /members/:id/colony-role`. That gap is now closed for both the email and phone paths.
...
```
A phone-granted row's `created` entry has no `email` key (same convention as a no-login row) — `login_granted: true` plus the row's own `phone` (already present) is how a client tells a phone-keyed login apart from none. `row` is 1-based and counts data rows only (excludes the header). `skipped` is specifically the duplicate-phone case — dedup is checked **only within the target colony's own roster** (the same `UNIQUE (colony_id, phone)` index single-create already relies on), not against unrelated unscoped legacy rows (a deliberate choice — see PROGRESS.md). Note this is a `members`-table-level check, distinct from and checked *before* any login-identifier check — a duplicate `phone` within the same colony's roster is always `skipped` regardless of `grant_login`, even if the underlying `users.phone` would otherwise have been free. `errors` covers everything else row-level: missing `name`/`phone`; an `email` already registered (against an existing account, or a duplicate within the same file) — surfaces as `"email already registered"`; and (new, migration 012) a `phone` that already backs another user's login account **globally** (`users.phone` is unique across all colonies, unlike `members.phone` which is only unique per-colony) — surfaces as `"phone already registered for login"`. Top-level 400s: missing `colony_id`/`initial_password`/`file`, or a `colony_id` that doesn't resolve to a real colony. 403 if the caller isn't that colony's admin.

**Grant login** (`POST /members/:id/grant-login`) body: `{ email, password }`. Response `201`: the updated member row (now with `user_id` set). 400 if either field missing. 404 if the member doesn't exist. 409 if the member already has a linked login, or if `email` is already registered to someone else. **Not extended by migration 012** — this single-grant endpoint and `bulkImportMembers`'s login-granting code were already two separate, unshared implementations before this change (not a common helper), so per explicit scope decision only the bulk path gained phone-login support. This endpoint stays email+password only, and does **not** auto-link colony membership the way the bulk path now does — flagged as a known inconsistency between the two, not fixed here.
...
**No pagination and no sort-order control anywhere in the API** — every list is `ORDER BY <primary key> ASC` with no override. Filtering is exact-match everywhere **except** four endpoints with a partial, case-insensitive `?search=`: `GET /users?search=` (email OR phone, migration 012), `GET /donors?search=` (name/phone), `GET /members?search=` (name/phone, combinable with `?colony_id=`), `GET /colonies?search=` (name/location). No other endpoint has free-text search.
...
- **Token contents:** `{ user_id, email, phone }` (migration 012 — `email`/`phone` may individually be `null`, depending which identifier the account has and which one was used to log in) plus standard JWT claims (`iat`, `exp`).
...
- **Member login-granting/promotion (migration 011, extended migration 012):** `POST /members/bulk` always requires `colony_id` and is always colony-admin-gated — no unscoped mode, unlike single-create. As of migration 012, it also grants a phone-keyed login (via the row's `grant_login` column when `email` is empty, see §4) in addition to the original email path, and — new — **auto-inserts a `colony_memberships` row (role `member`) for whichever user a row's login was granted to**, closing a gap where an email-granted bulk login previously never got colony write access. `POST /members/:id/grant-login` and `POST /members/:id/reset-password` reuse single-create's existing conditional rule instead of a new one: colony-admin required if the target member row has a `colony_id`, any authenticated user if it's a legacy unscoped row. **`grant-login` was deliberately not extended to phone or to the colony auto-link** in migration 012 — its login-granting code was never shared with `bulkImportMembers`'s (two separate implementations already), so per explicit scope decision only the bulk path changed; this is a known, flagged inconsistency between the two. `PATCH /members/:id/colony-role` does no authorization checks of its own at all — it's a thin dispatcher onto `colonyMembershipService.addMember`/`updateMemberRole`, which already enforce `assertColonyAdmin` (403) and the sole-admin-can't-be-demoted guard (400) against the `colony_id` given in the request body.
...
| Phone already backs another user's login (bulk import, `grant_login` phone path) | row-level `errors` entry | `"phone already registered for login"` | Caught Postgres `23505` on `users_phone_unique` (migration 012) — a *global* check, distinct from the per-colony `members_colony_id_phone_unique` check above, which is evaluated first and takes precedence for a same-colony duplicate |
...
**Data required:** Register: email, password. Login: password plus exactly one of email/phone (migration 012 — most self-registered users only have email, but a bulk-imported volunteer may only have a phone login, so a login form should offer both, not just email).
**APIs required:** `POST /auth/register`, `POST /auth/login`.
**Actions:** Submit registration (email+password only — there is no phone-based self-registration); submit login (email or phone); switch between the two forms.
...
**Actions:** Add/edit a member; bulk-upload a CSV/XLSX roster (colony-admin only) and show the created/skipped/error breakdown per row — a row's `email` column grants an email login, or (migration 012) a truthy `grant_login` column with no `email` grants a phone login instead, and either way the person is now automatically added to the colony (no separate promote step needed just to get them write access); grant a member login access via email or reset their password (colony-admin, or any user if the member is unscoped — this single-grant endpoint is still email-only, not extended to phone in migration 012); promote/demote a member with a linked login to colony admin (blocked with 400 if they have no login yet); view signups and availability calendar; toggle a day's availability.
